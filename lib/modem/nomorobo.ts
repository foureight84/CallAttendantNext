...
/**
 * Scrapes nomorobo.com to check if a number is a robocall.
 * Returns score >= config.spamThreshold to block.
 */
export class NomoroboChecker {
  async check(phoneNumber: string): Promise<NomoroboResult> {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(`[nomorobo] HTTP ${response.status} for ${phoneNumber}`);
